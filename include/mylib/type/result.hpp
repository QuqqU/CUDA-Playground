/*
   ...
   This header provides a type::_Result type that can be used to replace
   exceptions in code that has to handle error.

   type::_Result<T, E> can be used to return and propagate an error to the
   caller. type::_Result<T, E> is an algebraic data type that can either Ok(T)
   to represent success or Err(E) to represent an error.
*/
...
/*
   ...
   This header provides a _Result type that can be used to replace exceptions in
   code that has to handle error.

   _Result<T, E> can be used to return and propagate an error to the caller.
   _Result<T, E> is an algebraic data type that can either Ok(T) to represent
...
template <typename T, typename E> struct _Result;
...
template <typename T, typename E> struct ResultOkType<_Result<T, E>> {
    typedef T type;
};
...
template <typename Ret, typename Arg> struct Map<Ret(Arg)> {
    ...
    static_assert(
        !IsResult<Ret>::value,
        "Can not map a callback returning a _Result, use andThen instead");

    template <typename T, typename E, typename Func>
    static _Result<Ret, E> map(const _Result<T, E> &result, Func func) {
...
template <typename Arg> struct Map<void(Arg)> {
    ...
    template <typename T, typename E, typename Func>
    static _Result<void, E> map(const _Result<T, E> &result, Func func) {
...
// Specialization for a void _Result
template <typename Ret> struct Map<Ret(void)> {

    template <typename T, typename E, typename Func>
    static _Result<Ret, E> map(const _Result<T, E> &result, Func func) {
        static_assert(std::is_same<T, void>::value,
                      "Can not map a void callback on a non-void _Result");
...
// Specialization for callback returning void on a void _Result
...
// General specialization for a callback returning a _Result
template <typename U, typename E, typename Arg> struct Map<_Result<U, E>(Arg)> {

    template <typename T, typename Func>
    static _Result<U, E> map(const _Result<T, E> &result, Func func) {
        static_assert(std::is_same<T, Arg>::value ||
                          std::is_convertible<T, Arg>::value,
                      "Incompatible types detected");
...
// Specialization for a void callback returning a _Result
template <typename U, typename E> struct Map<_Result<U, E>(void)> {

    template <typename T, typename Func>
    static _Result<U, E> map(const _Result<T, E> &result, Func func) {
        static_assert(std::is_same<T, void>::value,
                      "Can not call a void-callback on a non-void _Result");
...
struct Map<Ret (Cls::*)(Arg) const> {
    ...
    static_assert(
        !IsResult<Ret>::value,
        "Can not map a callback returning a _Result, use orElse instead");

    template <typename T, typename E, typename Func>
    static _Result<T, Ret> map(const _Result<T, E> &result, Func func) {
        if (result.isErr()) {
    ...
    static _Result<void, Ret> map(const _Result<void, E> &result, Func func) {
        if (result.isErr()) {
            auto res = func(result.storage().template get<E>());
            return types::Err<Ret>(res);
        }
...
template <typename Ret, typename Arg> struct Then<Ret(Arg)> {
    ...
    template <typename T, typename E, typename Func>
    static _Result<T, E> then(const _Result<T, E> &result, Func func) {
        if (result.isOk()) {
            func(result.storage().template get<T>());
        }
        return result;
...
struct Else<_Result<T, F>(Arg)> {

    template <typename E, typename Func>
    static _Result<T, F> orElse(const _Result<T, E> &result, Func func) {
    ...
    template <typename E, typename Func>
    static _Result<void, F> orElse(const _Result<void, E> &result, Func func) {
        if (result.isErr()) {
            auto res = func(result.storage().template get<E>());
            return res;
        }
...
template <typename T, typename F> struct Else<_Result<T, F>(void)> {
...
template <typename Ret, typename Arg> struct Wise<Ret(Arg)> {
    ...
    template <typename T, typename E, typename Func>
    static _Result<T, E> otherwise(const _Result<T, E> &result, Func func) {
        static_assert(std::is_same<E, Arg>::value ||
                          std::is_convertible<E, Arg>::value,
                      "Incompatible types detected");
...
template <
    ...
    typename Ret = _Result<typename details::ResultOkType<
                               typename details::result_of<Func>::type>::type,
                           E>>
Ret map(const _Result<T, E> &result, Func func) {
    return ok::Map<Func>::map(result, func);
}

template <typename T, typename E, typename Func,
          typename Ret =
              _Result<T, typename details::ResultErrType<
                             typename details::result_of<Func>::type>::type>>
Ret mapError(const _Result<T, E> &result, Func func) {
    return err::Map<Func>::map(result, func);
}

template <typename T, typename E, typename Func>
_Result<T, E> then(const _Result<T, E> &result, Func func) {
    return And::Then<Func>::then(result, func);
}

template <typename T, typename E, typename Func>
_Result<T, E> otherwise(const _Result<T, E> &result, Func func) {
...
template <typename T, typename E, typename Func,
          typename Ret =
              _Result<T, typename details::ResultErrType<
                             typename details::result_of<Func>::type>::type>>
Ret orElse(const _Result<T, E> &result, Func func) {
...
template <typename T, typename E> struct _Result {
    ...
    _Result(types::Ok<T> ok) : ok_(true) { storage_.construct(std::move(ok)); }

    _Result(types::Err<E> err) : ok_(false) {
        storage_.construct(std::move(err));
    }

    _Result(_Result &&other) {
        if (other.isOk()) {
            details::Constructor<T, E>::move(std::move(other.storage_),
                                             storage_, details::ok_tag());
            ok_ = true;
        } else {
            details::Constructor<T, E>::move(std::move(other.storage_),
                                             storage_, details::err_tag());
            ok_ = false;
        }
    }

    _Result(const _Result &other) {
        if (other.isOk()) {
    ...
    template <typename Func,
              typename Ret =
                  _Result<typename details::ResultOkType<
                              typename details::result_of<Func>::type>::type,
                          E>>
    ...
    template <typename Func,
              typename Ret = _Result<
                  T, typename details::ResultErrType<
                         typename details::result_of<Func>::type>::type>>
    Ret mapError(Func func) const {
        return details::mapError(*this, func);
    }

    template <typename Func> _Result<T, E> then(Func func) const {
        return details::then(*this, func);
    }

    template <typename Func> _Result<T, E> otherwise(Func func) const {
    ...
    unwrap() const {
        ...
        std::fprintf(stderr, "Attempting to unwrap an error _Result\n");
        std::terminate();
    }
    ...
    E unwrapErr() const {
        ...
        std::fprintf(stderr, "Attempting to unwrapErr an ok _Result\n");
        std::terminate();
    }
...
template <typename T, typename E>
bool operator==(const _Result<T, E> &lhs, const _Result<T, E> &rhs) {
    static_assert(concept ::EqualityComparable<T>::value,
                  "T must be EqualityComparable for _Result to be comparable");
    static_assert(concept ::EqualityComparable<E>::value,
                  "E must be EqualityComparable for _Result to be comparable");
...
template <typename T, typename E>
bool operator==(const _Result<T, E> &lhs, types::Ok<T> ok) {
    static_assert(concept ::EqualityComparable<T>::value,
                  "T must be EqualityComparable for _Result to be comparable");
...
template <typename E>
bool operator==(const _Result<void, E> &lhs, types::Ok<void>) {
    return lhs.isOk();
}

template <typename T, typename E>
bool operator==(const _Result<T, E> &lhs, types::Err<E> err) {
    static_assert(concept ::EqualityComparable<E>::value,
                  "E must be EqualityComparable for _Result to be comparable");
    if (!lhs.isErr())
        return false;
...
namespace types {
// template <typename T> using Ok = types::Ok<T>;
// template <typename E> using Err = types::Err<E>;
template <typename T, typename E> using Result = _Result<T, E>;
} // namespace type
